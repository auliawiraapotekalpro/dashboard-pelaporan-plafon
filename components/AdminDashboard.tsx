        ...
        <div className="flex justify-center">
          <div className="bg-white p-1 rounded-full shadow-sm flex gap-1 border border-slate-200">
            {(['ALL', 'PENDING', 'SCHEDULED', 'FINISHED'] as AdminFilter[]).map((item) => (
              <button key={item} onClick={() => setFilter(item)} className={`px-6 py-1.5 rounded-full text-[10px] font-black tracking-widest transition-all ${filter === item ? 'bg-[#1e293b] text-white' : 'text-slate-400 hover:bg-slate-50'}`}>{item}</button>
            ))}
          </div>
        </div>
            ...
            <table className="w-full border-collapse min-w-[2000px]">
              <thead className="sticky top-0 z-10 bg-[#f8fafc] text-[#94a3b8] text-[9px] font-black uppercase tracking-widest text-left">
                <tr>
                  ...
                  <th className="px-4 py-4 border-b border-slate-100 w-48">DEPT</th>
                  <th className="px-4 py-4 border-b border-slate-100 w-32">PIC</th>
                  <th className="px-4 py-4 border-b border-slate-100 w-32">RENCANA</th>
                  <th className="px-4 py-4 border-b border-slate-100 w-32">TARGET</th>
                  <th className="px-4 py-4 border-b border-slate-100 w-48">AKSI</th>
                </tr>
              </thead>
                ...
                {filteredReports.map((report) => {
                  ...
                  return (
                    <tr key={report.id} className="hover:bg-slate-50/30 transition">
                      <td className="px-4 py-5 font-black text-[#5850ec] text-xs">{report.id}</td>
                      ...
                      <td className="px-4 py-5 text-[10px] font-bold text-slate-400 text-center">{report.date}</td>
                      <td className="px-4 py-5 font-black text-slate-800 text-xs uppercase leading-tight">{report.tokoName}</td>
                      <td className="px-4 py-5 text-[11px] text-slate-500 leading-relaxed">{report.indicator}</td>
                      <td className="px-4 py-5 text-center">
                         <div className={`inline-block px-2 py-1 rounded border text-[9px] font-black tracking-widest ${report.riskLevel.includes('P1') ? 'bg-red-50 text-red-500 border-red-100' : 'bg-blue-50 text-blue-500 border-blue-100'}`}>{report.riskLevel.split(' - ')[0]}</div>
                      </td>
                      <td className="px-4 py-5 text-[10px] text-slate-400 italic leading-snug">{report.businessImpact}</td>
                      <td className="px-4 py-5 text-[10px] font-semibold text-slate-700 leading-relaxed whitespace-pre-line text-justify pr-4">{report.recommendation}</td>
                      ...
                      <td className="px-4 py-5 font-black text-indigo-700 text-[10px] text-center uppercase tracking-tighter">{report.pic !== '-' ? report.pic : '-'}</td>
                      <td className="px-4 py-5"><input type="date" value={report.plannedDate === '-' ? '' : report.plannedDate} onChange={(e) => handleInlineChange(report.id, { plannedDate: e.target.value })} className="bg-white border border-slate-200 p-2 rounded-xl text-[10px] font-bold w-full outline-none"/></td>
                      <td className="px-4 py-5"><input type="date" value={report.targetDate === '-' ? '' : report.targetDate} onChange={(e) => handleInlineChange(report.id, { targetDate: e.target.value })} className="bg-white border border-slate-200 p-2 rounded-xl text-[10px] font-bold w-full outline-none"/></td>
                      <td className="px-4 py-5">
                         <div className="flex gap-1.5">
                            <button disabled={report.status !== ReportStatus.PENDING} onClick={() => handleSubmitAction(report)} className={`px-3 py-2 rounded-xl font-black text-[9px] tracking-widest ${report.status === ReportStatus.PENDING ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}>SUBMIT</button>
                            <button disabled={report.status === ReportStatus.COMPLETED} onClick={() => handleFinishAction(report)} className="px-3 py-2 bg-green-600 text-white rounded-xl font-black text-[9px] tracking-widest">DONE</button>
                         </div>
                      </td>
                    </tr>
                  );
